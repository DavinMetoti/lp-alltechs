...
import { ChevronRight, Radio, Calendar, User, ArrowRight, Mail, Tag } from "lucide-react";
...
export default function NewsPage() {
  ...
  useEffect(() => {
    async function loadNews() {
      ...
      try {
        ...
        const res = await fetch(`${cleanBase}v1/news?per_page=12`, {
          headers: {
            "X-Api-Key": apiKey,
  ...
  const categories = ["Semua", ...Array.from(new Set(news.map((n) => n.category).filter((c): c is string => Boolean(c))))];

  const filteredNews = activeCategory === "Semua"
    ? news
    : news.filter((n) => n.category && n.category.toLowerCase() === activeCategory.toLowerCase());
